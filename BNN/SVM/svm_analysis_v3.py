...
def load_train_data(file_dir, background_flag=1):
    ...
    # extract center and background RDS
    rds_center = disp_map_avg[:, :, 128:384]
    rds_bg = np.concatenate(
        [disp_map_avg[:, :, 0:128], disp_map_avg[:, :, 384:]], axis=2
    )

    # subtract rds_bg from rds_center
    rds_center = rds_center - rds_bg.mean(axis=2)[:, :, np.newaxis]

    # compute mean and std for normalization
    x_mean = rds_center.mean()  # mean of training dataset
    x_std = rds_center.std()  # std of training dataset

    # normalize and reshape data
    n_dotDens, n_train, _ = rds_center.shape
    X_train = []
    for dd in range(n_dotDens):
        for i in range(n_train):
            # if Y_train[dd, i] in label_to_train:
            disp_map_this = (rds_center[dd, i] - x_mean) / x_std
...
def load_test_data(file_dir, x_mean, x_std, background_flag=1):
    ...
    # normalize data
    X_ards = (rds_center - x_mean) / x_std
    ...
    # normalize data
    X_hmrds = (rds_center - x_mean) / x_std
