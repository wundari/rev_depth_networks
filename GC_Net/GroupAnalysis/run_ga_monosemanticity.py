...
# %% compute monosemantic score (monosemantic neuron / feature)
threshold = 0.9
n_feat_global = 27
n_feat_rds = 9
mono_score, mono_score_rds = ga_mono.compute_monosemantic_score(
    threshold, n_feat_global, n_feat_rds
)
# %% statistical test
ga_mono.stat_test(mono_score, mono_score_rds)

# %% plot n_mono per features for all interactions
ga_mono.plotLine_n_mono_vs_layer_all_interactions(mono_score, save_flag)

# %%
rds_types = ["ards", "hmrds", "crds"]
for rds_type in rds_types:
    ga_mono.plotLine_n_mono_vs_layer_all_interactions_rds(
        mono_score_rds, rds_type, save_flag
    )
