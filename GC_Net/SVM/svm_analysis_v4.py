...
def load_train_data(file_dir: str, background_flag: bool = 1):
    ...
    Returns:
        ...
        x_mean ([1, 1, w]]): mean of training dataset
        x_std ([1, 1, w]): std of training dataset
    """
    ...
    # compute mean and std for normalization across batch and dot density
    x_mean = disp_map_avg.mean(axis=(0, 1), keepdims=True)  # [1, 1, w]
    x_std = disp_map_avg.std(axis=(0, 1), keepdims=True)  # [1, 1, w]
    ...
    X_train = (
        disp_map_avg - x_mean
    ) / x_std  # [len(dotDens_list), len(disp_ct_pix) * n_rds_each_disp, w]
    Y_train = Y_train.reshape(
        disp_map_avg.shape[0], -1
    )  # [len(dotDens_list), len(disp_ct_pix) * n_rds_each_disp]
...
def load_test_data(
    ...
    Args:
        ...
        x_mean ([n_dotDens, 1, w]]): mean of training dataset
        x_std ([n_dotDens, 1, w]): std of training dataset
    ...
    # standardize data
    X_ards = (disp_map_avg - x_mean) / x_std
    ...
    # normalize data
    X_hmrds = (disp_map_avg - x_mean) / x_std
...
def xDecode_single_bootstrap(
    ...
    X_train_split = X_train[:, idx_train].reshape(
        -1, X_train.shape[-1]
    )  # [n_dotDens * n_train, w]
