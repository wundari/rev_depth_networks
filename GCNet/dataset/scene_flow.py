...
class SceneFlowFlyingThingsDataset(data.Dataset):
    def __init__(self, datadir, config, split="train"):
        ...
        self._read_data()
        self.left_data = split_sequences(self.left_data, config, split, flying=isinstance(self, SceneFlowFlyingThingsDataset))
        self._augmentation()
...
def _load_stereo_sample(dataset, idx):
    ...
    sample_seed = (dataset.augmentation_seed + int(dataset.epoch) * len(dataset) + idx
                   if dataset.split == "train" else dataset.split_seed + idx)
    ...
    result = random_crop(dataset.h_crop, dataset.w_crop, result,
                         dataset.c_disp_shift, dataset.split, reference=reference, rng=rng)
...
def split_sequences(paths, config, split, *, flying=False):
    ...
    selected = {"validation": groups[:nv], "test": groups[nv:nv+nt],
                "train": groups[nv+nt:]}[split]
